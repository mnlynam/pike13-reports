...
| Version | Date | Notes |
|---|---|---|
| 1.7 | 2026-04-08 | Fixed the Slack message length warning to fire at the correct threshold. v1.6 warned only when output exceeded 12,000 characters, but Slack's actual per-message limit is 4,000, so the warning never fired in the danger zone. The over-limit toast now names a specific remedy (use Canvas instead) rather than the previous vague "may exceed" wording. The Slack button's tooltip also gained a pre-emptive warning that wide tables won't fit. |
